...
async function getSwiss() {
    if (!swissPromise) {
        swissPromise = (async () => {
            // Unpkg exposes the class constructor directly on the global window scope
            const EphemerisEngine = window.SwissEph;
            
            if (!EphemerisEngine) {
                throw new Error("Swiss Ephemeris calculation library failed to mount globally.");
            }
